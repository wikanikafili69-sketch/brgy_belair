<?php
// API/Index/indegency_api_insert.php
...
try {
    ...
    // 1. Get prefix (Default to 'WI-' for Web Indigency)
    $prefix = isset($data['queue_prefix']) ? $data['queue_prefix'] : 'WI-';

    // 2. Count rows in the Indigency table to get the next number
    $count_sql = "SELECT COUNT(*) as total_rows FROM request_certificate_indigency";
    $count_stmt = $pdo->query($count_sql);
    $row_data = $count_stmt->fetch();
    
    $next_number = $row_data['total_rows'] + 1;
    $formatted_queue = $prefix . $next_number; // e.g., WI-1

    // 3. Insert into Universal Queue Ledger
    $queue_sql = "INSERT INTO service_queues (service_type, queue_number, record_status) 
                  VALUES ('Certificate of Indigency', :qno, 'for approval')";
    $queue_stmt = $pdo->prepare($queue_sql);
    $queue_stmt->execute([':qno' => $formatted_queue]);

    // 4. Insert Request Data into the Indigency Table
    ...
    $insert_sql = "INSERT INTO request_certificate_indigency (
        ...
        :fname, :addr, :ctype, :auth, 
        :house, :purpose, :other_purpose, :phone, :queue, 'for approval'
    )";
    ...
    $insert_stmt->execute([
        ...
        ':ctype'         => $data['certificate_type'],
        // These might be empty strings if not a claimant, which is fine
        ':auth'          => !empty($data['authorized_person']) ? $data['authorized_person'] : null, 
        ':house'         => !empty($data['household_type']) ? $data['household_type'] : null,
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Indigency request submitted.',
