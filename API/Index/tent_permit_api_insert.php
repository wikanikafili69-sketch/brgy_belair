<?php
// API/Index/tent_permit_api_insert.php
...
try {
    ...
    // 1. Get prefix (Default to 'OTP-' for Web Tent Permit)
    $prefix = isset($data['queue_prefix']) ? $data['queue_prefix'] : 'OTP-';

    // 2. Count rows in the Tent Permit table to get the next number
    $count_sql = "SELECT COUNT(*) as total_rows FROM request_certificate_tent_permit";
    $count_stmt = $pdo->query($count_sql);
    $row_data = $count_stmt->fetch();
    
    $next_number = $row_data['total_rows'] + 1;
    $formatted_queue = $prefix . $next_number; // e.g., OTP-1

    // 3. Insert into Universal Queue Ledger
    $queue_sql = "INSERT INTO service_queues (service_type, queue_number, record_status) 
                  VALUES ('Certificate of Tent Permit', :qno, 'for approval')";
    $queue_stmt = $pdo->prepare($queue_sql);
    $queue_stmt->execute([':qno' => $formatted_queue]);

    // 4. Insert Request Data into the Tent Permit Table
    // --> Added 'other_purpose_details'
    // --> Removed 'additional_contact_number' since we swapped it in the UI
    $insert_sql = "INSERT INTO request_certificate_tent_permit (
        ...
        :fname, :addr, :purpose, :other_purpose, 
        :phone, :date_used, :queue, 'for approval'
    )";
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Tent Permit request submitted.',
