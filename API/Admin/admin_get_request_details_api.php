...
try {
    // 1. FIRST: Get the master status from the service_queues table!
    // ✅ CHANGED: Now pulling 'record_status' instead of 'status'
    ...
    // ✅ CHANGED: Defaulting fallback to 'for approval'
    $master_status = $master_record ? $master_record['record_status'] : 'for approval';
    ...
    foreach ($tables_to_search as $table => $service_name) {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                // ✅ CHANGED: Assigning to 'record_status' so the frontend JS reads it properly
