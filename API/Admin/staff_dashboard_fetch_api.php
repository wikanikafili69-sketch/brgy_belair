...
try {
    ...
    $unlock_sql = "UPDATE service_queues 
                   SET status = 'pending', processed_by = NULL 
...
try {
    ...
    $where_clause = "WHERE DATE(created_at) = :fdate";
    $params = [':fdate' => $filter_date];

    // Filter by Status
    if ($filter_status !== 'all') {
        $where_clause .= " AND status = :fstatus";
    ...
    $sql = "SELECT queue_number, service_type, created_at, status, processed_by, priority 
    ...
    // --- GET REAL-TIME TOP SUMMARY CHIPS ---
    $status_summary = ['pending' => 0, 'processing' => 0, 'completed' => 0, 'cancelled' => 0];
    try {
        $status_sql = "SELECT status, COUNT(*) as total FROM service_queues WHERE DATE(created_at) = :fdate GROUP BY status";
    ...
    $type_summary = [];
    try {
        $type_sql = "SELECT service_type, COUNT(*) as total FROM service_queues WHERE DATE(created_at) = :fdate GROUP BY service_type";
