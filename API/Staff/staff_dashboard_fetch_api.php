...
try {
    ...
    $where_clause = "WHERE DATE(created_at) = :fdate AND LOWER(service_type) NOT LIKE '%barangay id%'" . $source_clause;
    ...
    try {
        $status_sql = "SELECT record_status, COUNT(*) as total FROM service_queues WHERE DATE(created_at) = :fdate AND LOWER(service_type) NOT LIKE '%barangay id%'" . $source_clause . " GROUP BY record_status";
    ...
    $type_summary = [];
    try {
        $type_sql = "SELECT service_type, COUNT(*) as total FROM service_queues WHERE DATE(created_at) = :fdate AND LOWER(service_type) NOT LIKE '%barangay id%'" . $source_clause . " GROUP BY service_type";
