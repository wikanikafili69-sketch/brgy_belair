...
// ==========================================
// 🚨 CRITICAL SECURITY CHECK: STAFF EDITING RIGHTS
// ==========================================
try {
    // Step A: Find the queue_number for this specific record_id
    $q_stmt = $pdo->prepare("SELECT queue_number FROM {$target_table} WHERE request_id = ?");
    $q_stmt->execute([$record_id]);
    ...
    $queue_number = $q_row['queue_number'];

    // Step B: Check the master status in service_queues (UPDATED to record_status)
    $status_stmt = $pdo->prepare("SELECT record_status FROM service_queues WHERE queue_number = ?");
    $status_stmt->execute([$queue_number]);
    $status_row = $status_stmt->fetch(PDO::FETCH_ASSOC);

    // Step C: Kill the script if it's not pending or processing
    if (!$status_row || !in_array(strtolower($status_row['record_status']), ['pending', 'processing'])) {
        echo json_encode(['success' => false, 'message' => 'Unauthorized: Documents can only be edited when Pending or Processing.']);
        exit;
    }
} catch (PDOException $e) {
    echo json_encode(['success' => false, 'message' => 'Security Validation Error: ' . $e->getMessage()]);
    exit;
}
// ==========================================
...
// ==========================================
// PHOTO PROCESSING (USING YOUR EXACT FORMAT)
// ==========================================
...
if (isset($fields['new_photo_base64']) && !empty($fields['new_photo_base64'])) {
    ...
    if (count($parts) == 2) {
        ...
        // ------------------------------------
        // GENERATE RANDOM CHARACTERS & DATE (Matched to submit_batch_api.php)
        // ------------------------------------
        ...
        // ------------------------------------
        // FINAL FILE NAME (NEW FORMAT)
        // Result: id_abcde_20260425_104.png
        // ------------------------------------
        $fileName = 'id_' . $randomChars . '_' . $dateNow . '_' . $record_id . '.png';
        ...
        // This will cleanly write the new file
...
// Remove base64 string from array
