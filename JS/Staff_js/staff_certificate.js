/**
 * staff_certificate.js
 * Professional AJAX Management with Locking, Multi-item Support, and Fullscreen View
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentItems = []; 
    let itemIndex = 0; 
    let currentWorkspaceReadOnly = false; 

    // API Routes
    const FETCH_API = '../API/Staff/staff_dashboard_fetch_api.php'; 
    const DETAILS_API = '../API/Staff/staff_get_request_details_api.php';
    const LOCK_API = '../API/Staff/staff_certificates_lock_api.php';
    const STATUS_API = '../API/Staff/staff_update_status_api.php';
    ...
    window.fetchTableData = function(page = 1, isSilent = false) {
        ...
        const source = document.getElementById('filterCertSource') ? document.getElementById('filterCertSource').value : 'all';

        if (!isSilent) document.getElementById('certTableBody').innerHTML = '<tr><td colspan="6" style="text-align:center; padding:40px;">Loading data...</td></tr>';

        const t = new Date().getTime();

        fetch(`${FETCH_API}?page=${currentPage}&limit=${perPage}&search=${search}&filter_source=${source}&filter_status=${status}&filter_type=${type}&t=${t}`)
            .then(res => res.json())
            .then(result => {
    ...
    function renderTableRows(data) {
        ...
        if (!data || data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:40px; color:var(--text-muted);">No requests match your filter for today.</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = data.map(row => {
            ...
            const statusDisplay = row.status || row.record_status || 'PENDING';
            const badgeClass = statusDisplay.toLowerCase().replace(/\s+/g, '-');
            ...
            return `
                <tr class="${trClass}" onclick="viewRequest('${row.queue_number}')" style="cursor:pointer; ${rowStyle}">
                    ...
                    <td><span class="source-badge source-badge--${row.queue_number.startsWith('W') ? 'walkin' : 'online'}">${row.queue_number.startsWith('W') ? 'Walk-in' : 'Online'}</span></td>
                    <td style="font-size:0.82rem; line-height: 1.4;">${formalTypeHTML}</td>
                    <td style="font-size:0.78rem;">${row.formatted_date}</td>
                    <td><span class="badge badge--${badgeClass}">${statusDisplay.toUpperCase()}</span></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function updateTypeChips(typeData) {
        ...
        const activeType = dropdown.value || 'all';
        ...
        container.querySelectorAll('.type-chip').forEach(chip => {
            chip.addEventListener('click', (e) => {
                dropdown.value = e.currentTarget.getAttribute('data-type');
                fetchTableData(1, false); 
            });
        });
    }

    // --- 2. THE LOCK MECHANISM & SEPARATE WORKSPACE ---
    window.claimRequest = function(queueNo, currentStatus) {
        currentWorkspaceReadOnly = false; 

        const executeLockAndOpen = () => {
            fetch(LOCK_API, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            .then(data => {
                if (data.success) {
                    logActivity(`Began processing request for Queue: <strong>${queueNo}</strong>.`);
        ...
        if (currentStatus === 'pending') {
            fetch(STATUS_API, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ queue_number: queueNo, record_status: 'processing' })
            })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    ...
                    alert("Failed to move request to processing: " + (data.message || "Unknown error"));
                }
            });
        } else {
            executeLockAndOpen();
    ...
    function setupWorkspaceModal(queueNo) {
        ...
        const requestStatus = currentItems[0].record_status ? currentItems[0].record_status.toLowerCase() : 'pending';
        ...
        const pendingBtn = document.getElementById('wsPendingBtn');
        const processBtn = document.getElementById('wsProcessBtn');
        const rejectBtn = document.getElementById('wsRejectBtn');
        const printBtn = document.getElementById('wsPrintAllBtn');
        const completeBtn = document.getElementById('wsCompleteBtn'); 
        ...
        if (currentWorkspaceReadOnly || requestStatus === 'completed' || requestStatus === 'rejected') {
            pendingBtn.style.display = 'inline-flex';
            pendingBtn.innerHTML = `<i class="fa-solid fa-xmark"></i> Close View`;
            pendingBtn.onclick = () => {
                modal.classList.add('hidden');
            };
        } else if (requestStatus === 'processing' || requestStatus === 'pending') {
            pendingBtn.style.display = 'inline-flex';
            pendingBtn.innerHTML = `<i class="fa-solid fa-clock-rotate-left"></i> Mark Pending`;
            pendingBtn.onclick = () => handleWorkspaceExit(queueNo, 'pending', 'Document reverted to Pending.');
            
            // 🔥 BULLETPROOF PRINT ALL DOCUMENTS LOGIC 🔥
          // 🔥 BULLETPROOF PRINT ALL DOCUMENTS LOGIC (IFRAME METHOD) 🔥
            printBtn.style.display = 'inline-flex';
            printBtn.onclick = () => {
                logActivity(`Printed documents for Queue: <strong>${queueNo}</strong>.`);

                let allCertsHTML = '';
                
                // 1. Generate HTML for EVERY document in the queue
                currentItems.forEach((item, index) => {
                    const sName = getFormalServiceName(item.service_type || item.internal_service_type);
                ...
                // 2. Create the hidden iframe to handle clean printing
                ...
                // 4. Write the HTML into the iframe document with strict A4 constraints
            ...
            if (completeBtn) {
                ...
                completeBtn.onclick = () => {
                    if(confirm("Are you sure you want to mark this request as Completed?")) {
                         handleWorkspaceExit(queueNo, 'completed', 'Request Completed successfully!');
                    }
                };
            }
        }
...
// 🔥 INDIVIDUAL PRINT LOGIC (IFRAME METHOD) 🔥
    window.printSingleDocument = function(index) {
        const item = currentItems[index];
        const sName = getFormalServiceName(item.service_type || item.internal_service_type);
        const formalDate = new Date().toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' });
        
        logActivity(`Printed individual document: <strong>${sName}</strong> for Queue <strong>${item.queue_number || 'Unknown'}</strong>.`);
    ...
    window.renderWorkspaceService = function(index) {
        ...
        const itemStatus = item.record_status ? String(item.record_status).toLowerCase() : 'pending';
        const isEditable = !currentWorkspaceReadOnly && (itemStatus === 'processing' || itemStatus === 'pending');

        const actionBarHTML = `
            <div class="no-print workspace-action-bar">
                <div class="workspace-action-title">
                    <i class="fa-solid fa-file-invoice"></i> Document Editor
                </div>
                <div class="workspace-action-buttons">
                
                    <button class="btn-ghost" onclick="printSingleDocument(${index})" style="border: 1px solid var(--border-light); padding: 8px 16px; border-radius: 6px; color: var(--navy); display: flex; align-items: center; gap: 6px; cursor: pointer; background: white;">
    ...
    window.saveServiceDetails = function(itemIndex) {
        ...
        fetch('../API/Staff/staff_update_service_details_api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            if (data.success) {
                ...
                logActivity(`Updated document fields for <strong>${serviceName}</strong> (Ref ID: ${recordId}).`);
    ...
    function handleWorkspaceExit(queueNo, targetStatus, successMsg) {
        fetch(STATUS_API, { 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ queue_number: queueNo, record_status: targetStatus })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                
                logActivity(`Updated status of Queue <strong>${queueNo}</strong> to <strong>${targetStatus.toUpperCase()}</strong>.`);

                document.getElementById('processWorkspaceModal').classList.add('hidden');
    ...
    function renderItemDetail() {
        ...
        const detailBadgeClass = itemStatus.replace(/\s+/g, '-');
        document.getElementById('detailStatusBadge').innerHTML = `<span class="badge badge--${detailBadgeClass}">${itemStatus.toUpperCase()}</span>`;
        ...
        renderActionButtons(itemStatus, item.queue_number);
    }
    ...
    function renderActionButtons(status, queueNo) {
        const container = document.getElementById('certActionButtons');
        
        if (status === 'pending') {
            container.innerHTML = `
                <button class="btn-cert-action btn-cert-process btn-cert-full" onclick="claimRequest('${queueNo}', 'pending')" style="margin-bottom: 8px;">
                    <i class="fa-solid fa-gears"></i> Process Workspace
                </button>
                <div style="display: flex; gap: 8px;">
                    <button class="btn-cert-action" style="flex: 1; background: #f1f5f9; color: var(--text-dark); border: 1px solid var(--border-light);" onclick="openReadOnlyWorkspace('${queueNo}')">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button class="btn-cert-action" style="flex: 1; background: #fee2e2; color: #ef4444; border: 1px solid #fca5a5;" onclick="updateStatus('${queueNo}', 'rejected')">
            ...
            container.innerHTML = `
                <button class="btn-cert-action btn-cert-process btn-cert-full" onclick="claimRequest('${queueNo}', 'processing')" style="margin-bottom: 8px;">
                    <i class="fa-solid fa-file-signature"></i> Open Workspace (Edit / Print)
                </button>
                <button class="btn-cert-action btn-cert-full" style="background: #f1f5f9; color: var(--text-dark); border: 1px solid var(--border-light);" onclick="updateStatus('${queueNo}', 'pending')">
                    <i class="fa-solid fa-clock-rotate-left"></i> Revert to Pending
                </button>
            `;
        } else {
            container.innerHTML = `
                ...
                <button class="btn-cert-action btn-cert-full" style="background: #f1f5f9; color: var(--text-dark); border: 1px solid var(--border-light); margin-bottom: 8px; cursor: pointer;" onclick="openReadOnlyWorkspace('${queueNo}')">
                    <i class="fa-solid fa-eye"></i> View Document (Read-Only)
                </button>
                <button class="btn-cert-action btn-cert-full" style="background: #e0e7ff; color: #4f46e5; border: 1px solid #c7d2fe; cursor: pointer;" onclick="updateStatus('${queueNo}', 'pending')">
                    <i class="fa-solid fa-clock-rotate-left"></i> Revert back to Pending
                </button>
            `;
        }
    }
    ...
    window.updateStatus = function(queueNo, newStatus) {
        if (!confirm(`Are you sure you want to change this request to ${newStatus.toUpperCase()}?`)) return;

        fetch(STATUS_API, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ queue_number: queueNo, record_status: newStatus })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {

                logActivity(`Changed status of Queue <strong>${queueNo}</strong> to <strong>${newStatus.toUpperCase()}</strong>.`);
    ...
    const filterSource = document.getElementById('filterCertSource');
    if (filterSource) {
        filterSource.addEventListener('change', () => fetchTableData(1, false));
    }
    
    document.getElementById('filterCertStatus').addEventListener('change', () => fetchTableData(1, false));
    document.getElementById('filterCertType').addEventListener('change', () => fetchTableData(1, false));
...
// ========================================
// STAFF CAMERA & SMART COMPRESSION LOGIC
// ========================================
