...
function initConcretePrompt() {

    // ── INJECT MODALS & UI ────────────────────────────────────
    buildToastUI(); 
    buildConcretePromptModal();
    // WE REMOVED buildCpVerificationModal() HERE! 
    buildConcreteFormModal();

    // ── CACHE REFS ────────────────────────────────────────────
    var promptOverlay = document.getElementById('cp-confirm-overlay');
    var btnYes        = document.getElementById('cp-confirm-yes');
    var btnNo         = document.getElementById('cp-confirm-no');
    var btnClose      = document.getElementById('cp-confirm-close');
    ...
    var successWrap   = document.getElementById('concrete-success-wrap');
        ...
        btnYes.addEventListener('click', function () {
            ...
            setTimeout(function() {
                ...
                ResidentVerification.open(function(userData) {
                    // Concrete form doesn't need to auto-fill the requestor's name, 
                    // so we just proceed to open the form directly!
                    setTimeout(openConcreteForm, 200);
                });
            }, 200);
        });
    }
    ...
    // ── FORM BACK / ESC ───────────────────────────────────
    if (formBack) formBack.addEventListener('click', closeConcreteForm);
    ...
    if (form) {
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            if (!validateConcreteForm()) return;
            ...
            const payload = {
                queue_prefix: 'OCP-',
                company_name: getVal('cp-company'),
                purpose: getVal('cp-purpose'),
                location: getVal('cp-location'),
                date_range: getVal('cp-date-range'), // Already perfectly formatted by Flatpickr!
                time_from: getVal('cp-time-from'),
                time_to: getVal('cp-time-to'),
                vehicles: getVal('cp-vehicles'),
                phone: getVal('cp-phone')
            };

            fetch('API/Index/concrete_api_insert.php', {
                method: 'POST',
            ...
            .then(function(data) {
                if (data.success) {
                    var ref = document.getElementById('concrete-ref-number');
    ...
    // ── SUCCESS SCREEN ────────────────────────────────────
    function showConcreteSuccess() {
        if (formWrap) formWrap.style.display = 'none';
        if (successWrap) successWrap.classList.add('visible');

        var doneBtn = document.getElementById('concrete-done-btn');
        if (doneBtn) {
            doneBtn.addEventListener('click', function () {
                closeConcreteForm();
                setTimeout(resetConcreteForm, 400);
            }, { once: true });
        }
    }
    ...
    function validateConcreteForm() {
        ...
        var dateRangeVal = getVal('cp-date-range');
        ...
        var phoneVal = getVal('cp-phone');
        if (phoneVal && !/^\d{11}$/.test(phoneVal)) {
            var phoneEl = document.getElementById('cp-phone');
            if (phoneEl) markConcreteError(phoneEl, 'Enter a valid 11-digit number.');
            valid = false;
        }
    ...
    // ── BUILD PROMPT HTML ─────────────────────────────────
    function buildConcretePromptModal() {
        var html = `
        <div class="rbi-confirm-overlay" id="cp-confirm-overlay" role="dialog" aria-modal="true">
            <div class="rbi-confirm-modal">
                <button class="rbi-confirm-close" id="cp-confirm-close" aria-label="Close">✕</button>
                <div class="rbi-confirm-icon">🏗️</div>
                <h2 class="rbi-confirm-title">Concrete Pouring Certification</h2>
                <p class="rbi-confirm-question">Have you already registered as a resident <strong>(RBI)</strong>?</p>
                <p class="rbi-confirm-sub">You must be registered in the Barangay Resident Information (RBI) system before you can request a Concrete Pouring Certification.</p>
                <div class="rbi-confirm-actions">
                    <button class="rbi-confirm-btn rbi-confirm-btn--no" id="cp-confirm-no">
                    ...
                    <button class="rbi-confirm-btn rbi-confirm-btn--yes" id="cp-confirm-yes">
                        <span class="rbi-confirm-btn-icon">✅</span>
                        <span class="rbi-confirm-btn-text"><strong>Yes, Proceed</strong><small>Request Concrete Pouring Certification</small></span>
                    </button>
                </div>
            </div>
        </div>`;
    ...
    // WE REMOVED THE ENTIRE buildCpVerificationModal() HTML BLOCK HERE!
    ...
    function buildConcreteFormModal() {
        ...
        <div class="residency-form-overlay" id="concrete-form-overlay" role="dialog" aria-modal="true">
            ...
            <div id="concrete-form-wrap">
                ...
                <form class="rbi-form-body residency-form-body" id="concrete-request-form" novalidate>
                    ...
                    <div class="rbi-row rbi-row-3">
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-company">
                                Company Name: <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-company" class="rbi-input"
                                   data-required="true" placeholder="COMPANY NAME" maxlength="150">
                            <span class="cp-error-msg"></span>
                        </div>
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-purpose">
                                Purpose: <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-purpose" class="rbi-input"
                                   data-required="true" placeholder="PURPOSE" maxlength="200">
                            <span class="cp-error-msg"></span>
                        </div>
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-location">
                                Location: <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-location" class="rbi-input"
                                   data-required="true"
                    ...
                    <div class="rbi-row rbi-row-3">
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-date-range">
                                Date (from-to): <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-date-range" class="rbi-input"
                                   data-required="true" placeholder="Select Start & End Date" readonly>
                            <span class="cp-error-msg"></span>
                        </div>
                        <div class="cp-field">
                            ...
                            <div class="cp-time-range">
                                <input type="time" id="cp-time-from" class="rbi-input cp-time-input"
                                       data-required="true">
                                <span class="cp-time-sep">to</span>
                                <input type="time" id="cp-time-to" class="rbi-input cp-time-input"
                                       data-required="true">
                            </div>
                            <span class="cp-error-msg" id="cp-time-error"></span>
                        </div>
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-vehicles">
                                Vehicle(s): <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-vehicles" class="rbi-input"
                                   data-required="true" placeholder="VEHICLE(S)" maxlength="200">
                            <span class="cp-error-msg"></span>
                        </div>
                    </div>
                    ...
                    <div class="rbi-row rbi-row-2">
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-certificate">
                                Certificate: <span class="req">*</span>
                            </label>
                            <input type="text" id="cp-certificate" class="rbi-input readonly-field"
                                   value="CONCRETE POURING CERTIFICATION" readonly>
                            <span class="cp-error-msg"></span>
                        </div>
                        <div class="cp-field">
                            <label class="rbi-label" for="cp-phone">
                                Cellphone Number: <span class="req">*</span>
                            </label>
                            <input type="tel" id="cp-phone" class="rbi-input"
                    ...
                    <div class="rbi-submit-row">
                        <button type="submit" id="concrete-submit-btn" class="rbi-submit-btn" disabled>
            ...
            <div class="rbi-success" id="concrete-success-wrap">
                <div class="rbi-success-icon">✅</div>
                <h2 class="rbi-success-title">Request Submitted!</h2>
                <p class="rbi-success-sub">
                    Your Concrete Pouring Certification request has been submitted successfully.<br>
                    Please wait for confirmation from the Barangay Hall.
                </p>
                <div class="rbi-success-ref">Reference No: <span id="concrete-ref-number">—</span></div>
                <button id="concrete-done-btn" class="rbi-submit-btn">
