// JS/Services_templates/residency_certificate.js

function getResidencyHTML(data, dateStr) {
    ...
    // --- SMART PURPOSE LOGIC ---
    const definedPurposes = ["Employment", "Loan Application", "School Enrollment", "Government Benefit", "Legal Transaction", "Travel / Visa", "Other", ""];
    ...
    if (isOther && data.purpose !== 'Other' && !otherValue) {
        otherValue = data.purpose; // Fallback for custom purposes
    }
    ...
    const editForm = `
        <div id="editFormPanel_${recordId}" class="edit-form-panel no-print hidden" style="background: #ffffff; padding: 20px; border: 1px solid var(--border-light); border-radius: 8px; margin-bottom: 20px; text-align: left;">
            <h4 style="margin: 0 0 16px 0; font-size: 1rem; color: var(--navy); border-bottom: 1px solid var(--border-light); padding-bottom: 10px;">
                <i class="fa-solid fa-house-user" style="color: var(--blue); margin-right: 6px;"></i> Edit Residency Details
            </h4>
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <select name="purpose" class="edit-input" data-required="true" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px; background: white;" onchange="let w = document.getElementById('wrap_other_purpose_${recordId}'); let i = document.getElementById('input_other_purpose_${recordId}'); if(this.value === 'Other') { w.style.display = 'block'; } else { w.style.display = 'none'; i.value = ''; }">
                        ...
                        <option value="Employment" ${displayPurpose === 'Employment' ? 'selected' : ''}>Employment</option>
                        <option value="Loan Application" ${displayPurpose === 'Loan Application' ? 'selected' : ''}>Loan Application</option>
                        <option value="School Enrollment" ${displayPurpose === 'School Enrollment' ? 'selected' : ''}>School Enrollment</option>
                        <option value="Government Benefit" ${displayPurpose === 'Government Benefit' ? 'selected' : ''}>Government Benefit</option>
                        <option value="Legal Transaction" ${displayPurpose === 'Legal Transaction' ? 'selected' : ''}>Legal Transaction</option>
                        <option value="Travel / Visa" ${displayPurpose === 'Travel / Visa' ? 'selected' : ''}>Travel / Visa</option>
    ...
    <div class="certificate-paper with-watermark">
        ...
        <div class="cert-body">
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                ...
                whose name and signature appear below, is a citizen of the Republic of the Philippines and an adult resident of 
            ...
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                This certification is being issued upon his/her request for 
                ...
                or any lawful purpose it may serve.
            </p>
