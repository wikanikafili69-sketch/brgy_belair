// JS/Services_templates/low_income_certificate.js

function getLowIncomeHTML(data, dateStr) {
    ...
    // --- SMART PURPOSE LOGIC ---
    const definedPurposes = ["Financial Assistance", "Scholarship", "Medical Subsidy", "Educational Assistance", "Government Subsidy", "Other", ""];
    ...
    const workDetails = data.work_details || 'N/A';
    const fullPurpose = (data.purpose || '') + ' ' + otherValue;
    const incomeFormatted = parseFloat(data.income_amount || 0).toLocaleString('en-US', {minimumFractionDigits: 2});

    // BOX STYLE: This forces the underline to align perfectly with the surrounding text baseline
    ...
    const workBoxStyle = `${boxStyleBase} min-width: 150px; max-width: 250px;`;
    const workTextStyle = `white-space: normal; word-break: break-word; font-size: ${workDetails.length > 25 ? '0.75rem' : '1.15rem'};`;

    const incomeBoxStyle = `${boxStyleBase} min-width: 100px; max-width: 200px;`;
    const incomeTextStyle = `white-space: normal; word-break: break-word; font-size: 1.15rem;`;
    ...
    const editForm = `
        <div id="editFormPanel_${recordId}" class="edit-form-panel no-print hidden" style="background: #ffffff; padding: 20px; border: 1px solid var(--border-light); border-radius: 8px; margin-bottom: 20px; text-align: left;">
            ...
            <h4 style="margin: 0 0 16px 0; font-size: 1rem; color: var(--navy); border-bottom: 1px solid var(--border-light); padding-bottom: 10px;">
                <i class="fa-solid fa-hand-holding-dollar" style="color: var(--blue); margin-right: 6px;"></i> Edit Low Income Details
            </h4>

            <div style="font-size: 0.75rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 10px;">Resident Information</div>
            ...
            <div style="font-size: 0.75rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 10px;">Employment & Income Details</div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                <div>
                    <label style="font-size: 0.8rem; font-weight: 700; color: var(--text-mid); display: block; margin-bottom: 4px;">WORK/JOB DETAILS <span style="color: red;">*</span></label>
                    <input type="text" name="work_details" class="edit-input" data-required="true" value="${data.work_details || ''}" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;" placeholder="e.g. Construction Worker / Unemployed">
                </div>
                <div>
                    <label style="font-size: 0.8rem; font-weight: 700; color: var(--text-mid); display: block; margin-bottom: 4px;">MONTHLY INCOME (Php) <span style="color: red;">*</span></label>
                    <input type="number" step="0.01" name="income_amount" class="edit-input" data-required="true" value="${data.income_amount || 0}" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
                </div>
            </div>

            <div style="font-size: 0.75rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 10px;">Request Purpose</div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <select name="purpose" class="edit-input" data-required="true" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px; background: white;" onchange="let w = document.getElementById('wrap_other_purpose_${recordId}'); let i = document.getElementById('input_other_purpose_${recordId}'); if(this.value === 'Other') { w.style.display = 'block'; } else { w.style.display = 'none'; i.value = ''; }">
                        ...
                        <option value="Financial Assistance" ${displayPurpose === 'Financial Assistance' ? 'selected' : ''}>Financial Assistance</option>
                        <option value="Scholarship" ${displayPurpose === 'Scholarship' ? 'selected' : ''}>Scholarship</option>
                        <option value="Medical Subsidy" ${displayPurpose === 'Medical Subsidy' ? 'selected' : ''}>Medical Subsidy</option>
                        <option value="Educational Assistance" ${displayPurpose === 'Educational Assistance' ? 'selected' : ''}>Educational Assistance</option>
                        <option value="Government Subsidy" ${displayPurpose === 'Government Subsidy' ? 'selected' : ''}>Government Subsidy</option>
    ...
    <div class="certificate-paper with-watermark">
        ...
        <div class="cert-body">
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                ...
                and is known to me as a person of good moral character.
            </p>
            
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                Based on the records and to the best of my knowledge, the aforementioned resident is currently employed as/engaged in 
                <span style="${workBoxStyle}">
                    <strong id="lbl_work_details_${recordId}" style="${workTextStyle} text-transform: uppercase;">${workDetails}</strong>
                </span> 
                with an estimated declared monthly income of Php 
                <span style="${incomeBoxStyle}">
                    <strong id="lbl_income_amount_${recordId}" style="${incomeTextStyle}">${incomeFormatted}</strong>
                </span>.
            </p>
