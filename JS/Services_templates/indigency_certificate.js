// JS/Services_templates/indigency_certificate.js

function getIndigencyHTML(data, dateStr) {
    ...
    // --- SMART PURPOSE LOGIC ---
    const definedPurposes = ["Medical Assistance", "Financial Assistance", "Scholarship / Educational Assistance", "Burial Assistance", "Hospital Requirement", "Other", ""];
    ...
    const editForm = `
        <div id="editFormPanel_${recordId}" class="edit-form-panel no-print hidden" style="background: #ffffff; padding: 20px; border: 1px solid var(--border-light); border-radius: 8px; margin-bottom: 20px; text-align: left;">
            ...
            <h4 style="margin: 0 0 16px 0; font-size: 1rem; color: var(--navy); border-bottom: 1px solid var(--border-light); padding-bottom: 10px;">
                <i class="fa-solid fa-file-invoice" style="color: var(--blue); margin-right: 6px;"></i> Edit Indigency Details
            </h4>
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <select name="purpose" class="edit-input" data-required="true" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px; background: white;" onchange="let w = document.getElementById('wrap_other_purpose_${recordId}'); let i = document.getElementById('input_other_purpose_${recordId}'); if(this.value === 'Other') { w.style.display = 'block'; } else { w.style.display = 'none'; i.value = ''; }">
                        ...
                        <option value="Medical Assistance" ${displayPurpose === 'Medical Assistance' ? 'selected' : ''}>Medical Assistance</option>
                        <option value="Financial Assistance" ${displayPurpose === 'Financial Assistance' ? 'selected' : ''}>Financial Assistance</option>
                        <option value="Scholarship / Educational Assistance" ${displayPurpose === 'Scholarship / Educational Assistance' ? 'selected' : ''}>Scholarship / Educational Assistance</option>
                        <option value="Burial Assistance" ${displayPurpose === 'Burial Assistance' ? 'selected' : ''}>Burial Assistance</option>
                        <option value="Hospital Requirement" ${displayPurpose === 'Hospital Requirement' ? 'selected' : ''}>Hospital Requirement</option>
    ...
    <div class="certificate-paper with-watermark">
        ...
        <div class="cert-title" style="margin-bottom: 15px;">
            ${data.certificate_type || 'CERTIFICATE OF INDIGENCY'}
        </div>
        ...
        <div class="cert-body">
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                ...
                and is personally known to me as a peace-loving citizen with good moral character.
            </p>
            
            <p style="text-indent: 50px; margin-bottom: 15px; line-height: 2.2;">
                This further certifies that the above-named individual belongs to the indigent and marginalized families in this barangay.
            </p>
